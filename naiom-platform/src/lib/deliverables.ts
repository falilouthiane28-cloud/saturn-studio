...
export async function listAllDeliverables(): Promise<(Deliverable & { agentSlug: AgentSlug })[]> {
  // Dérivé du registre : un agent ajouté ou retiré est pris en compte ici
  // sans qu'on ait à maintenir une seconde liste.
  const slugs = AGENT_SLUGS;
