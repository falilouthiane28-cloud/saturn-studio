...
export function parseDeckMarkdown(markdown: string, fallbackTitle = "Présentation"): Presentation {
  ...
  slides.push({
    ...
    category: (fmBrand ?? "Saturn Studio").toUpperCase(),
  });
  ...
  if (!hasThanks && slides.length > 3) {
    slides.push({
      kind: "thanks",
      title: "merci",
      body: "Cette présentation a été générée automatiquement par la plateforme Saturn Studio.",
      resources: ["saturnagency.com"],
    });
  }

  return {
    title: deckTitle,
    brand: fmBrand ?? "Saturn Studio",
    date: fmDate,
    slides,
  };
}
