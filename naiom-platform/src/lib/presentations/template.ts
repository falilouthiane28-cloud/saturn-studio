...
export interface SlideTitle {
  kind: "title";
  category?: string; // petit texte en haut (ex. "Saturn Studio")
  title: string;
  subtitle?: string;
}
...
export interface Presentation {
  ...
  brand?: string; // footer droit — défaut "Saturn Studio"
  date?: string; // footer gauche — défaut mois/année courant
  slides: Slide[];
}

// ---- Couleurs Saturn Studio (dérivées du template client) ----
const COLOR = {
  bg: "#E8E4DD",
  ink: "#121212",
  accent: "#6D28D9", // orange primaire
...
function renderFooter(i: number, total: number, p: Presentation): string {
  ...
  const brand = (p.brand ?? "Saturn Studio").toUpperCase();
...
function renderTitle(s: SlideTitle, i: number, total: number, p: Presentation): string {
  ...
  return `<section class="slide slide-title">
    ...
    <div class="title-category">${escapeHtml(s.category ?? (p.brand ?? "Saturn Studio").toUpperCase())}</div>
