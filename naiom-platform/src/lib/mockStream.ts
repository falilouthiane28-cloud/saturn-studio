...
# Brief de campagne — Lancement l'Agent SDR (Agent IA SDR)

**Fenêtre** : 4 semaines · **Budget média** : 0 € (organique) · **Canaux** : LinkedIn + YouTube

## Contexte & objectif

Saturn Studio lance **l'Agent SDR**, son agent IA de prospection, sur son propre marketing (dogfooding). Objectif SMART : **10 RDV qualifiés Calendly + 5 demandes d'intégration similaires** en 4 semaines, 100 % organique.
...
**"Votre meilleur commercial vient de poser 3 semaines de vacances. l'Agent SDR, lui, travaille cette nuit."**

## 3 angles de contenu

1. **"Le SDR qui ne prend jamais de congés"** (PAS, LinkedIn long, voix Fallou)
2. **"ChatGPT est un outil. l'Agent SDR est un collègue."** (Hook-Story-Offer, carrousel, voix l'equipe)
3. **"Je vous montre l'Agent SDR en prod"** (AIDA, vidéo YouTube + 3 shorts)

## Cadence hebdo

- Fallou : 2-3 posts LinkedIn + 1 short YouTube
- l'equipe : 2 posts LinkedIn
- Compte Saturn Studio : 2 posts (long + carrousel)

## KPIs cibles (4 semaines)

- 80 000+ impressions LinkedIn cumulées
- 500+ nouveaux followers (Fallou + l'equipe + page)
- 50+ DM/commentaires "l'Agent SDR"
...
> **Prochaine étape** : enregistrez ce brief comme livrable (bouton en bas à gauche), puis demandez-moi les contenus — je les produirai angle par angle.`,
  ...
  strategiste: `# Brief de campagne — Lancement l'Agent SDR (Agent IA SDR)

**Client** : Saturn Studio · **Fenêtre** : 4 semaines · **Budget média** : 0 € (organique)

## 1. Contexte & objectif

Saturn Studio lance **l'Agent SDR**, son agent IA de prospection commerciale autonome, sur son propre marketing (dogfooding). Objectif SMART : **10 RDV qualifiés bookés sur Calendly + 5 demandes d'intégration "l'Agent SDR-like" en 4 semaines**, 100 % organique.

## 2. Analyse marché & concurrence

- 34 % des PME françaises utilisent déjà l'IA (France Num 2025)
- Concurrence positionnée sur l'outil (copilot) — Saturn Studio se positionne sur l'autonomie (agent qui tourne 24/7)
...
1. **"Le SDR qui ne prend jamais de congés"** (PAS) — post LinkedIn long, voix Fallou
2. **"ChatGPT est un outil. l'Agent SDR est un collègue."** (Hook-Story-Offer) — carrousel, voix l'equipe
3. **"Je vous montre l'Agent SDR en prod"** (AIDA) — vidéo YouTube + 3 shorts dérivés

## 6. Cadence hebdomadaire

| Profil | Lun | Mar | Mer | Jeu | Ven |
|---|---|---|---|---|---|
| Fallou | 1 post | — | 1 short YT | 1 thread | — |
| l'equipe | — | 1 carrousel | — | 1 post | — |
| Compte Saturn Studio | 1 post | — | 1 Reel | — | 1 carrousel |

## 7. KPIs cibles (4 semaines)

- Impressions LinkedIn : 80 000+
- Nouveaux followers cumul (Fallou + l'equipe + page) : 500+
- DM / commentaires "l'Agent SDR" : 50+
...
**Framework** : PAS (Problem-Agitate-Solution) · **Voix** : Fallou (CTO) · **Format** : LinkedIn long
...
Il s'appelle l'Agent SDR.

Et non, ce n'est pas un humain — c'est un agent IA qu'on a construit chez Saturn Studio.
...
l'Agent SDR n'est pas un ChatGPT déguisé. C'est un agent orchestré sur n8n, branché sur vos données, avec des garde-fous Human-in-the-Loop sur les envois sensibles.
...
Un client nous a confié récemment : *"l'Agent SDR has completely changed our outbound strategy — our booking rate tripled in less than 30 days."*

C'est **leur** résultat, documenté. Pas une promesse Saturn Studio. Mais ça donne une idée de ce qu'un agent bien construit peut débloquer.

## CTA

> Je prépare une démo courte de l'Agent SDR (pas encore publique). Commentez **"l'Agent SDR"** ou envoyez-moi un DM pour la recevoir en avant-première.
...
- Post publié depuis le profil personnel de Fallou (CTO Saturn Studio)
- Audience cible : dirigeants PME B2B francophones (vouvoiement maintenu)
- Créneau optimal suggéré : lundi matin 7h45-8h30`,

  designer: `# Prompts visuels — Campagne l'Agent SDR (3 formats)
...
## 1. Hero campagne (16:9, cover LinkedIn page Saturn Studio + thumbnail YouTube)
...
\`\`\`
Same style sheet as Variante A. New framing: square composition, tight on the monitor with the filling calendar, split by a vertical line. Left side: dark navy background with large white typography reading "l'Agent SDR". Right side: photograph of the monitor. Same palette, same photographic treatment. Aspect ratio 1:1.
\`\`\`
...
\`\`\`
Vertical portrait editorial poster. Top half: photograph of empty startup desk at night (navy lighting, single blush lamp). Bottom half: large serif typography "23:47" in nude pink #F5D5CB on navy #1E2957, with small caption "l'Agent SDR travaille" in clean sans-serif white below. Magic prompt off. Aspect ratio 9:16.
\`\`\`
...
**Client** : Saturn Studio · **Canaux** : LinkedIn (Fallou + l'equipe + page) · **Période** : S16 2026
...
- **DM entrants "l'Agent SDR"** : 14 (objectif hebdo : 10) — ✅ objectif dépassé
- **Alerte** : chute de reach sur le profil de l'equipe (-18 %), probablement liée à une semaine sans post
...
1. **"Le SDR qui ne prend jamais de congés"** (Fallou, 15/04) — 12 400 impressions, 8,3 % engagement
2. **Carrousel "ChatGPT vs l'Agent SDR"** (l'equipe, 16/04) — 5 200 impressions, 7,1 % engagement
3. **Court YouTube "Démo l'Agent SDR en 60s"** (Fallou, 17/04) — 2 800 vues, 42 % rétention

## Flop (à analyser)

- **Post de l'equipe du 13/04** ("3 idées fausses sur l'IA") — 340 impressions seulement, 2,1 % engagement. Hypothèse : hook faible + pas de preuve sociale intégrée.
...
### YouTube
- **CTR miniature** : 8,2 % sur le court "Démo l'Agent SDR" (très bon, >5 % = excellent)
- **À ajuster** : audio du short un peu faible vs ambiance

## Recommandations (priorité haute → basse)

1. **Re-publier Angle A** sur LinkedIn page Saturn Studio (avec crédit Fallou) — la reach personnelle est saturée, la page amplifie
2. **Écrire le "B side"** du post Angle A : témoignage client développé (Warburg.ai)
3. **l'equipe** : doubler la cadence la semaine prochaine pour compenser la chute
...
- Pas encore d'exports Calendly pour corréler DM → RDV bookés (l'equipe doit remonter le CSV)
...
- Les chiffres ci-dessus sont calculés sur des exports mockés pour cette démo ; dans l'usage réel, l'agent se connecte à LinkedIn / YouTube / Calendly via n8n et tire les chiffres en direct.`,
  ...
  presentateur: `# Deck pitch prospect — "Mettez l'Agent SDR au travail"
...
**Layout** : plein cadre photo (Ideogram 16:9) + logo Saturn Studio en haut à droite
**Visuel** : hero campagne l'Agent SDR (voir prompts-images, variante IDE-1)
**Durée orale** : 30 s
...
**Durée orale** : 30 s
**Notes orateur** : pause théâtrale. Enchaîner sur l'Agent SDR slide 5.

## Slide 5 — Réponse : l'Agent SDR

**Copy** : "l'Agent SDR : un agent IA qui prospecte, qualifie et relance 24/7."
**Layout** : 3 colonnes d'icônes (Prospection / Qualification / Relance) + logo Saturn Studio
**Durée orale** : 120 s
...
**Layout** : tableau 3 colonnes (agences trad / no-code / Saturn Studio)
**Durée orale** : 60 s
...
- [ ] Palette : bleu marine \`#1E2957\` + rose nude \`#F5D5CB\`
- [ ] Logo Saturn Studio en pied de chaque slide
...
- **Sacha Navette (Warburg.ai)** — Bug l'Agent SDR ce matin : 12 messages LinkedIn envoyés avec l'ancien template. Appel client critique à 14h. **→ Action : call Sacha avant 12h, fix workflow n8n v1 qui tournait en parallèle.**

## 🟠 Important (sous 24h)

- **Julie Lefort (Agence Lefort)** — Prospect chaud, 45 personnes, budget 5-8k€. A vu le post l'Agent SDR. **→ Action : répondre avec 3 créneaux cette semaine + envoyer deck l'Agent SDR version immo.**
- **l'Agent Executif Makri (Agence SLM)** — Demande de décalage de call jeudi (enfant malade). **→ Action : proposer 2 créneaux semaine prochaine.**
- **l'equipe** — Review deck webinaire v3 (slides 12-18 + nouveau slide pricing). **→ Action : bloquer 45 min ce week-end.**

## 🟢 À lire (info)

- **Baptiste Legue (Piscineôpropre)** — Feedback 3 mois Alex (positif, demande 2 évolutions). Pas urgent mais répondre avant vendredi.
- **Thomas Vuillemin (VCM Conseil)** — Demande tarifs l'Agent Executif pour cabinet de 25 personnes. l'equipe est sur le dossier.
...
- **3 actions équipe** avec l'equipe
- **1 client existant** à rassurer (l'Agent Executif) et **1 à écouter** (Baptiste)
...
**Contexte** : Agence immobilière Lyon, 45 personnes, CA 8M€. Besoin l'Agent SDR pour qualification de leads. Budget 5-8k€ setup + 1k€/mois retainer. Décision dans 2 semaines.

**Plan d'action équipe** :

| Action | Responsable | Échéance | Priorité |
|---|---|---|---|
| Envoyer pitch deck adapté immobilier | Fallou | Vendredi 24/04 | 🔴 |
| Chiffrage 3 scénarios | l'equipe | Lundi 27/04 | 🔴 |
| Démo live avec DSI Alex Brun | Fallou + l'equipe | 28/04 à 10h | 🟠 |
...
**Contexte** : Cabinet B2B 25 personnes. Intéressé par l'Agent Executif (AI Executive Assistant) pour toute l'équipe. Amélie (RH) très engagée. Budget annuel évoqué : ~15k€ "tech productivité".

**Plan d'action équipe** :

| Action | Responsable | Échéance | Priorité |
|---|---|---|---|
| Devis avec option formation (workshops + playbooks) | l'equipe | 22/04 | 🔴 |
| Confirmer l'Agent Executif fonctionne en n8n on-premise | Fallou | 22/04 | 🔴 |
| Relancer si pas de retour | l'equipe | 28/04 | 🟠 |

### 🟡 Call-003 — Sacha Navette (Warburg.ai) — **incident résolu, relation à renforcer**

**Contexte** : Bug l'Agent SDR ce matin (12 messages avec ancien template). Root cause : workflow v1 non désactivé. Fix pendant le call.

**Plan d'action équipe** :

| Action | Responsable | Échéance | Priorité |
|---|---|---|---|
| Documenter process change management | Fallou | 27/04 | 🔴 |
| Geste commercial (1 mois offert sur mai) | l'equipe | Aujourd'hui | 🟠 |
| Ajouter alerting automatique sur déploiements | Fallou | Cette semaine | 🟠 |

## Calls informatifs (pas d'action urgente)

- **Call-004 — Samuel Redondo (LeLaboStore)** : signé, kick-off 27/04. Contrat envoyé.
- **Call-005 — 1:1 interne Fallou/l'equipe** : décision acte = pas de nouveau client avant fin avril (focus plateforme).
...
- Contenu technique (blog, talks) = bon pour le rayonnement Saturn Studio.
- Prétention salariale (65k€ sur 4j/sem) = **dans la range**.

**Réserves**
- Pas d'XP à très fort volume (>1k req/j) — mais on est plus orientés valeur que volume chez Saturn Studio.
- Demande 4j/semaine — à discuter (politique équipe actuelle : 5j).

**Recommandation** : entretien technique avec Fallou semaine du 27/04. Prévoir exercice n8n + RAG en live-coding. Si le 4j/sem est bloquant pour nous, le dire tôt.
...
**Recommandation** : appel découverte 30 min avec l'equipe pour tester la flexibilité (salaire + remote). Si les deux sont bloquants : refus poli avec mise en réserve pour un futur poste senior.
...
**Objet** : Entretien technique Saturn Studio — créneau semaine du 27/04 ?
...
**Objet** : Candidature Saturn Studio — suite
