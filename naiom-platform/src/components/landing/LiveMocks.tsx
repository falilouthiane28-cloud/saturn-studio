 ...
 *    titre "✳ Quoi de prévu, Fallou ?", barre de prompt, badge Opus 4.8)
 *  - le chat de la plateforme Saturn Studio (conversation avec un agent)
...
export function FileWindow({
  ...
  const color = (c?: FileLine["c"]) =>
    c === "head" ? "#A78BFA" : c === "dim" ? "#7C7597" : c === "key" ? "#8FD0FF" : c === "ok" ? "#7BE3A9" : "#E8E4FA";
  return (
...
// Mascottes des six agents réels. Plus de figurines génériques : un slug
// absent de cette table n'est plus un agent.
const AVATARS: Record<string, string> = {
  fireflies: "/agents/fireflies-avatar-256.webp",
  prospection: "/agents/prospection-avatar-256.webp",
  proposition: "/agents/proposition-avatar-256.webp",
  "createur-contenu": "/agents/createur-contenu-avatar-256.webp",
  veille: "/agents/veille-avatar-256.webp",
  ecommerce: "/agents/ecommerce-avatar-256.webp",
};
...
export function AgentPic({ slug, x, y, size = 60 }: { slug: string; x: number; y: number; size?: number }) {
  return (
    <image
      href={AVATARS[slug] ?? AVATARS.fireflies}
      x={x}
      y={y}
      width={size}
      height={size}
...
export function ClaudeWin({ x, y, s = 1, prompt, caption, children }: WinProps) {
  return (
    <g transform={`translate(${x} ${y}) scale(${s})`}>
      ...
      <text x="36" y="322" fontSize="12" fill="#8A867C">Fallou · Max</text>
      {/* titre principal */}
      <text x="188" y="86" fontSize="21" fontWeight="700" fill="#241F1A">
        <tspan fill="#E0764B">✳ </tspan>Quoi de prévu, Fallou ?
      </text>
...
export function ChatWin({ x, y, s = 1, agent = "Ousmane", slug = "fireflies", caption, children }: ChatProps) {
  return (
    <g transform={`translate(${x} ${y}) scale(${s})`}>
      ...
      <circle cx="416" cy="290" r="20" fill="var(--color-primary)" />
  ...
  return (
    <g transform={`translate(${x} ${y}) scale(${s})`}>
      ...
      {items.map((it, i) => {
        ...
        const row = (
          <g key={it.name}>
            {it.em && <rect x="10" y={ry - 4} width="330" height="38" rx="10" fill="#EDE4FD" />}
