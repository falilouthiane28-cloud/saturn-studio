...
const CHANNEL_META: Record<Channel, { icon: string; bg: string; ink: string }> = {
  LinkedIn: { icon: "Briefcase", bg: "rgba(214, 224, 244, 0.55)", ink: "#3D4E70" },
  Instagram: { icon: "Camera", bg: "rgba(244, 220, 232, 0.55)", ink: "#8C3E5C" },
  YouTube: { icon: "CirclePlay", bg: "rgba(244, 218, 218, 0.55)", ink: "#883333" },
    ...
    <div className="overflow-hidden rounded-2xl bg-white">
      ...
      <div className="px-6 py-4 border-b border-[var(--color-line)]">
        <div className="flex items-end justify-between gap-4 flex-wrap">
          <div>
            <div className="althea-eyebrow mb-1">— Semaine en cours</div>
            <h3 className="althea-headline" style={{ fontSize: "clamp(20px, 2vw, 26px)", lineHeight: 1.1 }}>
              {slots.length === 0 ? (
                <>
                  Semaine du {DAYS[0]} — <em>aucune publication planifiée.</em>
                </>
              ) : (
                <>
                  Semaine du {DAYS[0]} —{" "}
                  <em>
                    {slots.length} publication{slots.length > 1 ? "s" : ""}.
                  </em>
                </>
              )}
            </h3>
