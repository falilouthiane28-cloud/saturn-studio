...
interface AgentTabsProps {
   ...
   * sur "chat" plutôt que d'afficher un panneau vide.
   */
  initialTab?: string;
}
...
export function AgentTabs({
  ...
  // `null` = l'utilisateur n'a encore rien cliqué, l'URL fait foi.
  ...
  // Dérivé plutôt que stocké dans un effet : l'onglet demandé est affiché dès
  // le premier rendu (pas de passage par « Chat »), et le clic reprend la main.
  const tab: TabKey =
    pickedTab ??
    (tabs.some((t) => t.key === initialTab) ? (initialTab as TabKey) : "chat");
...
function HistoryTab({ agentSlug, onResume }: { agentSlug: string; onResume: () => void }) {
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem(`saturn-chat:${agentSlug}`);
      if (!raw) {
        setMsgs([]);
        return;
      }
