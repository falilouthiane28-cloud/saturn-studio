...
export async function Sidebar({ activeSlug }: { activeSlug?: string }) {
  ...
  const counts = await Promise.all(
    agents.map((a) =>
      a.status === "active" ? countDeliverables(a.slug) : Promise.resolve(0)
    )
  );
  ...
  return (
    <aside className="sticky top-4 h-[calc(100vh-2rem)] w-72 shrink-0 ml-4 flex flex-col overflow-hidden rounded-3xl glass-brutal z-20">
      <div className="flex items-center gap-3 px-5 py-5 border-b border-white/10">
        <div
          className="flex h-11 w-11 items-center justify-center rounded-xl border border-white/10 text-xl font-black shadow-[0_8px_24px_-8px_rgba(0,0,0,0.5)]"
          style={{
        ...
        <span style={{ color: "var(--text-1)" }}>
          <SaturnLogo variant="full" size={16} />
        </span>
      </div>

      <nav className="flex-1 overflow-y-auto px-2.5 py-3 space-y-1">
        {/* Une seule section : l'équipe tient en six. La découper en
            « marketing » / « opérations » par tranches d'index était fragile —
            retirer un agent décalait silencieusement les autres de groupe. */}
        <SidebarSection title={`L'équipe · ${agents.length}`}>
          {agents.map((a, i) => (
            <AgentLink
              key={a.slug}
              agent={a}
