...
export async function POST(req: Request) {
  ...
  try {
    ...
    const { text } = await generateText({
      ...
      prompt: `Tu es Awa, l'agent prospection de Saturn Studio (agence d'ingénierie d'agents IA et d'automatisations n8n pour PME, basée à Dubaï, clientèle francophone).
...
Règles STRICTES :
- Email : objet ≤ 8 mots, corps ≤ 120 mots, vouvoiement. 1re phrase = un fait PRÉCIS sur eux (leurs avis, leur activité, leur ville — pas de flatterie générique). 1 seul bénéfice concret lié à leur métier. CTA doux : proposer l'audit gratuit ou une question ouverte. Signature "Fallou — Saturn Studio".
